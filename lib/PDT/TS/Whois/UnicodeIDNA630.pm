...
=head2 is_pvalid( $char )

Test if B<$char> is PVALID.

    is_pvalid('a') or croak 'assertion error';

B<$char> must be a single character string.
...
=head2 is_contextj( $char )

Test if B<$char> is CONTEXTJ.

    is_contextj('a') or croak 'assertion error';
...
=head2 is_contexto( $char )

Test if B<$char> is CONTEXTO.

    is_contexto('a') or croak 'assertion error';
...
sub _parse_range {
    my $range = shift;
    if ( $range =~ /^([0-9A-F]{4,6})-([0-9A-F]{4,6})$/ ) {
        return ( hex( $1 ), hex( $2 ) );
    }
    elsif ( $range =~ /^([0-9A-F]{4,6})$/ ) {
        my $codepoint = hex( $1 );
        return ( $codepoint, $codepoint );
    }
    else {
        croak "Can't parse range '$range'";
    }
}

{
    ...
    while ( my $line = <DATA> ) {
        chomp $line;
        $csv->parse( $line ) or croak "Can't parse DATA: " . Text::CSV->error_diag();
        my ( $range, $property, undef ) = $csv->fields();
        my ( $first, $last ) = _parse_range( $range );

        if ( exists $properties{$property} ) {
            for ( my $codepoint = $first ; $codepoint <= $last ; $codepoint++ ) {
                my $char = pack( 'U', $codepoint );
                $properties{$property}{$char} = 1;
            }
        }
    }
    eof DATA or croak "Can't read DATA";
}
...
A copy of https://www.iana.org/assignments/idna-tables-6.3.0/idna-tables-properties.csv

See this URL for context: https://www.iana.org/assignments/idna-tables-6.3.0/idna-tables-6.3.0.xhtml
...
058F,DISALLOWED,ARMENIAN DRAM SIGN
...
0600-0604,DISALLOWED,ARABIC NUMBER SIGN..ARABIC SIGN SAMVAT
0605,UNASSIGNED,<RESERVED>
...
08A0,PVALID,ARABIC LETTER BEH WITH SMALL V BELOW
08A1,UNASSIGNED,<RESERVED>
08A2-08AC,PVALID,ARABIC LETTER JEEM WITH TWO DOTS ABOVE..ARABIC LETTER ROHINGYA YEH
08AD-08E3,UNASSIGNED,<RESERVED>..<RESERVED>
08E4-08FE,PVALID,ARABIC CURLY FATHA..ARABIC DAMMA WITH DOT
08FF,UNASSIGNED,<RESERVED>
0900-0957,PVALID,DEVANAGARI SIGN INVERTED CANDRABINDU..DEVANAGARI VOWEL SIGN UUE
...
0971-0977,PVALID,DEVANAGARI SIGN HIGH SPACING DOT..DEVANAGARI LETTER UUE
...
1CC0-1CC7,DISALLOWED,SUNDANESE PUNCTUATION BINDU SURYA..SUNDANESE PUNCTUATION BINDU BA SATANGA
...
1CD4-1CF6,PVALID,VEDIC SIGN YAJURVEDIC MIDLINE SVARITA..VEDIC SIGN UPADHMANIYA
...
1DFC-1DFF,PVALID,COMBINING DOUBLE INVERTED BREVE BELOW..COMBINING RIGHT ARROWHEAD AND DOWN ARROWHEAD BELOW
...
20A0-20BA,DISALLOWED,EURO-CURRENCY SIGN..TURKISH LIRA SIGN
...
2460-26FF,DISALLOWED,CIRCLED DIGIT ONE..WHITE FLAG WITH HORIZONTAL MIDDLE BLACK STRIPE
2700,UNASSIGNED,<RESERVED>
2701-2B4C,DISALLOWED,UPPER BLADE SCISSORS..RIGHTWARDS ARROW ABOVE REVERSE TILDE OPERATOR
...
4E00-9FCC,PVALID,"<CJK IDEOGRAPH, FIRST>..<CJK IDEOGRAPH, LAST>"
...
A69F-A6E5,PVALID,COMBINING CYRILLIC LETTER IOTIFIED E..BAMUM LETTER KI
...
A78E,PVALID,LATIN SMALL LETTER L WITH RETROFLEX HOOK AND BELT
...
A793,PVALID,LATIN SMALL LETTER C WITH BAR
...
A7AA,DISALLOWED,LATIN CAPITAL LETTER H WITH HOOK
...
111D0-111D9,PVALID,SHARADA DIGIT ZERO..SHARADA DIGIT NINE
...
1B000-1B001,PVALID,KATAKANA LETTER ARCHAIC E..HIRAGANA LETTER ARCHAIC YE
...
1F0B1-1F0BE,DISALLOWED,PLAYING CARD ACE OF HEARTS..PLAYING CARD KING OF HEARTS
...
1F0D1-1F0DF,DISALLOWED,PLAYING CARD ACE OF CLUBS..PLAYING CARD WHITE JOKER
...
1F130-1F16B,DISALLOWED,SQUARED LATIN CAPITAL LETTER A..RAISED MD SIGN
1F16C-1F16F,UNASSIGNED,<RESERVED>..<RESERVED>
1F170-1F19A,DISALLOWED,NEGATIVE SQUARED LATIN CAPITAL LETTER A..SQUARED VS
...
1F210-1F23A,DISALLOWED,SQUARED CJK UNIFIED IDEOGRAPH-624B..SQUARED CJK UNIFIED IDEOGRAPH-55B6
